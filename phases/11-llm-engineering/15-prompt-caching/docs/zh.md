...
```python
# 步骤 1：Anthropic 提示缓存 —— 使用 cache_control 标记显式缓存系统提示
import anthropic

client = anthropic.Anthropic()

# 系统提示带 cache_control 标记：告诉 Anthropic 缓存此块 5 分钟
SYSTEM = [
    {
        "type": "text",
...
def review(code: str):
    return client.messages.create(
        ...
        max_tokens=1024,
        system=SYSTEM,  # 稳定的系统提示在前，可变的用户消息在后
...
```python
# 1 小时扩展 TTL：写入溢价 2 倍，但对批处理作业回报快
...
```python
# 步骤 3：OpenAI 自动缓存 —— 无需配置，超过 1024 token 的前缀匹配自动获得 50% 折扣
...
resp = client.chat.completions.create(
    model="gpt-5",
    messages=[
        {"role": "system", "content": SYSTEM_PROMPT},   # 长且稳定的系统提示放在最前
        {"role": "user", "content": user_msg},           # 可变的用户消息放在最后
    ],
)
resp.usage.prompt_tokens_details.cached_tokens  # 查看折扣部分的 token 数
```
...
```python
# 步骤 4：Gemini 显式上下文缓存 —— 创建命名的缓存对象，按存储时长计费
from google import genai
from google.genai import types

client = genai.Client()

# 创建缓存：指定模型、系统指令、内容和 TTL
cache = client.caches.create(
    model="gemini-3-pro",
    config=types.CreateCachedContentConfig(
        display_name="rubric-v3",          # 缓存的显示名称
        system_instruction=RUBRIC,         # 稳定的系统指令
        contents=[FEW_SHOT_EXAMPLES],      # 可复用的少样本示例
        ttl="3600s",                       # 1 小时过期
    ),
)

# 使用缓存生成内容：通过 cache.name 引用已缓存的内容
